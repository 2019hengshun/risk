...
<el-button  type="primary" @click="reset">重置</el-button>
                    ...
                    <el-col :span="10" >
                        <el-row>
                            ...
                            <el-col :span="4">
                                 <el-button type="primary" icon="search" @click="handleSearch" class="l20" >搜索</el-button> 
                            </el-col>    
                        </el-row>
                    </el-col>
                </el-row>
            ...
            <el-container   :style="{ height: fullscreenHeight}" style="border: 1px solid #eee">
              <el-header style="font-size: 12px;overflow：hidden;line-height:60px;    background-color:#EEEEEE">
                <el-row >
                  ...
                  <el-col :span="2">
                    <el-button type="primary" @click="toPrint">打印报告</el-button>
                  </el-col>
...
export default {
  ...
  methods: {
    ...
    _httpGetZMCReport(arr) {
      this.tableData.length = 0;

      for (let a = 0; a < arr.length; a++) {
        console.log(arr[a]);
        let zmcReportId = arr[a].zhimaReportId;
        httpGetZMCReport(zmcReportId)
          .then(res => {
            arr[a].chbzmf = res.data.score;
            this.tableData.push(arr[a]);
          })
          .catch();
      }
      this.loading = false;
    ...
    handleReport(index, row) {
      ...
      httpPostCarryReport(mifengReportId)
        .then(res => {
          ...
          _this.userBasicInformation["cellLoc"] = row.phonenume;
        ...
        .then(res => {
          ...
          if (data.list == 0) {
            ...
            for (let a = 0; a < data.list.length; a++) {
              //   Object.assign(_this.tableData[a], { chbzmf: 3 });
              //   this.loading = false;
              httpGetZMCReport(data.list[a].zhimaReportId)
                .then(res => {
                  Object.assign(data.list[a], { chbzmf: res.data.score });
                  console.log(a);
                  if (a == data.list.length - 1) {
                    _this.tableData = JSON.parse(JSON.stringify(data.list));
                    _this.loading = false;
                  }
                })
                .catch();
          ...
          //   for (let a = 0; a < tableDat.length; a++) {
          //     console.log(tableDat[a]);
          //     let zmcReportId = tableDat[a].zhimaReportId;
          //     httpGetZMCReport(zmcReportId)
          //       .then(res => {
          //         tableDat[a].chbzmf = res.data.score;
          //         arr.push(tableDat[a]);
          //       })
          //       .catch();
          //   }

          //    _this.tableData = JSON.parse(JSON.stringify(tableDat));

          console.log(_this.tableData);
          //   _this.tableData = JSON.parse(JSON.stringify(tableData));
