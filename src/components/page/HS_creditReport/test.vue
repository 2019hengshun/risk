...
<script>
import { httpALLGetTDReport, httpGetList, httpGetTDReport } from "@/api/http";
...
export default {
  ...
  methods: {
    ...
    getTableData(pagesize, npage, time, phonenume, interfaceCallType = 4) {
      ...
      _this.loading = true;
      if (time.length === 0) {
        httpALLGetTDReport(pagesize, npage, "", "", phonenume, 4)
          .then(res => {
            let data = res.data;
        ...
        httpALLGetTDReport(pagesize, npage, ...time, phonenume, 4)
          .then(res => {
            let data = res.data;
            _this.allpage = data.allsize;
            _this.currentPage = data.npage;
